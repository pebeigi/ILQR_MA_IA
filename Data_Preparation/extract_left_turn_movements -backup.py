...
def extract_cases(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    rules_by_intersection = {}
    for rule in RULES:
        rules_by_intersection.setdefault(rule.intersection_id, []).append(rule)
    ...
    for vehicle_id, vehicle in df.groupby("id", sort=False):
        ...
        for idx, through_run in enumerate(runs):
            if not through_run.label.startswith("J"):
                continue

            intersection_id = int(through_run.label[1:])
            rules = rules_by_intersection.get(intersection_id)
            if not rules or idx == 0 or idx == len(runs) - 1:
                continue

            approach_run = runs[idx - 1]
            exit_run = runs[idx + 1]

            for rule in rules:
                if approach_run.label != rule.approach_label or exit_run.label != rule.exit_label:
                    continue

                case_id = f"{vehicle_id}_{rule.intersection_id}_{idx}_{rule.movement_name.replace(' ', '_')}"
                ...
                case_records.append(
                    {
                        ...
                        "junction_lane_kf_first": through_run.lane,
                        "junction_lane_kf_sequence": ",".join(
                            map(str, segment.loc[segment["lane_kf"].isin(JUNCTION_ZONES[rule.intersection_id]), "lane_kf"].drop_duplicates())
                        ),
                        "exit_group": rule.exit_group,
                        "exit_lane_kf": exit_run.lane,
                        "start_time": approach_run.start_time,
                        "intersection_entry_time": through_run.start_time,
                        "intersection_exit_time": through_run.end_time,
...
def plot_all_vehicle_trajectories(df: pd.DataFrame, reference_image_path: Path | None) -> Path:
    ...
    ax.set_title("All motor-vehicle trajectories")
    ax.set_xlabel("xloc_kf (m)")
    ax.set_ylabel("image-space y (m)" if image_height_m is not None else "yloc_kf (m)")
    if image_width_m is not None and image_height_m is not None:
        ax.set_xlim(0, image_width_m)
